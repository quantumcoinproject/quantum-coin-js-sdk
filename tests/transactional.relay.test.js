...
// These are the "transactional" tests using relay endpoints from examples.
// Per requirements:
// - Relay transactional tests must use the relay endpoints shown in examples
// - The transaction must either succeed OR fail only due to no coins for gas.
...
function isInsufficientGasErrorText(text) {
  if (!text || typeof text !== 'string') return false;
  const t = text.toLowerCase();
  return t.includes('insufficient') && t.includes('fund') && t.includes('gas');
}
...
describe('transactional', { concurrency: 1 }, () => {
  test(
    'relay submit sendCoins: must succeed or fail only for insufficient gas',
    { timeout: 120_000 },
    async () => {
      const initOk = await qcsdk.initialize(
      ...
      // Send 0 coins so the only acceptable failure is "no gas".
      const send = await qcsdk.sendCoins(wallet, TO_ADDRESS_EXAMPLE, '0', nonce);
      assert.ok(send);

      if (send.resultCode === 0) {
        // Accepted by relay
        return;
      }

      // Only acceptable failure is insufficient gas.
      const statusText = send.response?.statusText ?? '';
      const bodyText = await readResponseTextSafe(send.response);
      const combined = `${statusText}\n${bodyText}`.trim();

      assert.ok(
        isInsufficientGasErrorText(combined),
        `Transactional relay test failed with unexpected error. resultCode=${send.resultCode}\n${combined || '(no response body)'}`,
      );
    },
  );

...
// These are the "transactional" tests: they attempt to submit a transaction.
// Per requirements:
// - RPC-based transactional tests must use `https://public.rpc.quantumcoinapi.com`
...
function isInsufficientGasErrorMessage(message) {
  ...
  return (
    (m.includes('insufficient') && m.includes('fund')) &&
    m.includes('gas')
  );
}
...
describe('transactional', { concurrency: 1 }, () => {
  test(
    ...
    async () => {
      ...
      assert.ok(txnCount && (txnCount.result || txnCount.error));
      assert.ok(txnCount.result, `RPC error: ${txnCount.error?.message ?? 'unknown'}`);
      const nonce = parseInt(txnCount.result, 16);
      assert.ok(Number.isInteger(nonce) && nonce >= 0);

      // Build and sign a minimal transfer with value 0 to keep "value" out of the equation.
      ...
      assert.equal(sign.resultCode, 0);
      ...
      if (send && send.result && typeof send.result === 'string') {
        assert.ok(send.result.startsWith('0x'));
        return;
      }

      console.log(sign.txnData);

      // If it failed, ONLY accept "insufficient funds for gas" style errors.
      const msg = send?.error?.message ?? '';
      assert.ok(
        isInsufficientGasErrorMessage(msg),
        `Transactional RPC test failed with unexpected error: ${msg || JSON.stringify(send)}`,
      );
    },
  );
});
